"""Goal: create broker-confirmed cumulative ``Aₙ สะสม (USD)`` values.

Quick Start:
    pip install pandas numpy
    python step_16_actual_cumulative.py --raw raw.csv --previous step_15.csv --fix-c 1500 --output step_16.csv
...
GOAL = "สะสมเงินจริงตามเวลา โดย pending/rejected/unfilled ไม่เปลี่ยนยอด"
COLUMN = "Aₙ สะสม (USD)"
EXECUTION_PRICES = (
...
def transform(raw: pd.DataFrame, previous: pd.DataFrame, fix_c: float):
    ...
    return ledger["actual_cumulative"].round(2), ("แถวก่อน confirmed fill แรกจะว่างโดยตั้งใจ",), {
