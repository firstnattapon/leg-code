"""Goal: create broker-confirmed ``Rₙ อ้างอิง (USD)`` values.

Quick Start:
    pip install pandas numpy
    python step_14_reference.py --raw raw.csv --previous step_13.csv --fix-c 1500 --output step_14.csv

The file is complete and standalone. Quotes never replace a fill price.
...
GOAL = "คำนวณ Rₙ = FIX_C × ln(P_exec/P₀_exec) จาก execution ที่ยืนยันแล้วเท่านั้น"
...
def transform(raw: pd.DataFrame, previous: pd.DataFrame, fix_c: float):
    ...
    return ledger["ln_reference"].round(2), (message,), {"execution_anchor_p0": p0}
